...
- [x] Optional: same entry on Spark **Profile** tab (`ProfileScreen` + `AccountSessionSection`)
- [x] Persist `signOut` / switch-account immediately (`persistSessionPatch` → `savePersistedState`)
